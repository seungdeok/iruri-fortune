...
파이프라인의 자동 커밋을 메인 체크아웃과 분리하려고 **전용 worktree 하나**를 씁니다.
git 기본 기능이라 pipeline.py 가 감싸지 않아요 — 최초 한 번만:

```bash
git worktree add ../iruri-harness   # 옆에 harness 전용 작업트리 생성
cd ../iruri-harness                 # 앞으로 여기서 파이프라인을 돌려요
```

이후 모든 사이클은 이 디렉토리 안에서 돌아가고, phase 마다 새 `feat-<slug>` 브랜치를
이 worktree 안에서 갈아끼워요(= PR 1개/phase). 정리는 `git worktree remove ../iruri-harness`.
...
```bash
# 1. phase 시작 (feat-share-fortune 브랜치 체크아웃 + phase.json 생성)
